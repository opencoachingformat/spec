...
:schema-url: https://opencoachingformat.org/schema/v2.json
...
No open standard exists for team-sport coaching diagrams.
...
. A flat action-sequence model for multi-step animated drills
...
| *Actions are the single source of truth*
| Every position is derived from `entities`/`balls` setup plus the effect of every action an actor has had so far — no separate per-phase state anchor. Framing (grouping actions into a still) is entirely a renderer decision.
...
| `left_corner` y     | 13.98 | 43.305| 43.305| 38.305
| `free_throw_line` y | 8.20  | 28.0  | 28.0  | 23.0
| `left_elbow` x      | ±2.45 |  ±8.0 |  ±6.0 |  ±6.0
|===

NOTE: These are centered-coordinate values (y measured from the court center,
...
Two distinct entities MUST NOT occupy the same position. In the top-level
`entities` array (the document's initial setup), no two entities may share
coordinates that resolve closer than a **minimum separation** to each other.
...
|===
| Court Profile | Minimum separation
...
| `cut` | no | `moves[]` | `variant` (backdoor, give_and_go, flash, v_cut, l_cut, curl, flare, fade, basket); `intensity`; action-level `side`, `arc` (defaults); per-move `around_player`, `off_screen_by`, `intensity`, `side`, `arc`
...
* A carried ball is drawn slightly **ahead of and to one side of** the carrier —
  ...
  side is legible. "Ahead" is the direction of that ball-handling action
  (`dribble` / `pass` / `shoot`).
...
(and ball, for `dribble`) to a waypoint. The final `to` in the sequence becomes
the player's resulting position for every action that follows — there is no
separate state anchor to keep in sync with it. Reference fields (`around_player`,
...
The frame-level `duration_ms` field from the pre-v2.0.0 frame model is
removed; `intensity` is now the only way to express tempo.
...
| `court`           | yes | Court profile, court type, drill focus.
...
| `unit`          | `m`, `ft`
| Action `type` | move, cut, screen, defend, dribble, pass, shoot, rebound, pickup, tackle, clear, faceoff, check (valid subset depends on `sport` — see the Sport section)
...
Court Profile: FIBA, Half Court, Offense
Title: "3-Man Weave"
...
Output only valid JSON matching https://opencoachingformat.org/schema/v2.json
----
...
=== v2.0.0-alpha.1 (in progress — not a final v2.0.0 release)
...
**Breaking.** `sport` becomes a required top-level field — no default, no
back-compat handling for an absent value (RFC 0007).

**Breaking.** `court.ruleset` is renamed to `court.court_profile`
throughout the schema, spec, and registry data — the old name promised
game rules the field never held; it was always pure court geometry
(RFC 0012).

**Breaking.** `sport` now gates `court.court_profile` via a whitelist,
and `court.custom_dimensions` becomes sport-scoped (a soccer document's
`custom_dimensions` now offers soccer-shaped fields like `goal_width`,
never basketball's `paint_width`/`basket_from_baseline`). Named positions
gain a `not_applicable` exclusion mechanism, distinguishing "not
applicable under this court profile" from "unknown / typo." The registry
data moves from flat `sports/<sport>-v*.json` + `positions/<ruleset>-
v1.json` files to a `sports/<sport>/` bundle (a `sport.json` manifest
plus `court_profiles/<name>.json` per court profile) (RFC 0010).
...
Initial pre-stable draft — a line-based geometric format (movement/passing/
screen lines, entity delta states per frame). Superseded by the v1.0.0
semantic action model; retained here for historical context only.
